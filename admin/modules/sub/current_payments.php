...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            	<h4 class="title">Current and Last Week Payments Statistics</h4>
            	
            	                    <div class="alert alert-info alert-with-icon" data-notify="container">
	                                    <span data-notify="icon" class="ti-bell"></span>
	                                    <span data-notify="message">Pending & Rejected task are not include in this payments Statistics.</span>
	                                </div>
									            ...
									            <?php
									             //current week sql queries
                                                      ...
                                                      $sql_prepare = 'SELECT taggers.tagr_id, sum(tasks.attributes),sum(tasks.total_tags),sum(tasks.a_w_m),sum(tasks.t_w_m),sum(tasks.usd), taggers.username 
                                                                       FROM taggers INNER JOIN tasks ON taggers.tagr_id=tasks.tagr_id WHERE tasks.c_date BETWEEN "'.$dFrom_current.'" AND "'.$dTo_current.'" AND tasks.tagr_id=? AND taggers.tagr_type=1 AND taggers.status=1';  //join table tagger and task for getting tags do$dFrom_currentne by each taggger so far
	                        ...
	                        <div class="card">
	                            <div class="card-content">
                                    ...
                                    <div class="fresh-datatables">
										...
										<tr class="info">	
											<td colspan="9">	
											        <h6 class="title">Current Week Payments Statistics 
											            <span style="  text-transform: capitalize;">
											                (<?php  echo date(' l jS F Y', strtotime($dFrom_current)).' TO '.date(' l jS F Y', strtotime($dTo_current)); ?>)
											                </span>
											        </h6>
											 </td>
										   ...
										   <?php
                            	                ...
                            	                $stmts = $con->prepare('SELECT task_id FROM tasks WHERE c_date BETWEEN "'.$dFrom_current.'" AND "'.$dTo_current.'" AND tagr_id=?'); //counting the number of task done by each tagger so far
									...
									while($stmt_query->fetch())
										   ...
										   ?>
    										...
    										<tr class="info">	
    											<td colspan="9"><h6 class="title">Last Week Payments Statistics
    											  <span style="  text-transform: capitalize;">
    											                (<?php  echo date(' l jS F Y', strtotime($dFrom)).' TO '.date(' l jS F Y', strtotime($dTo)); ?>)
    											                </span>
										 ...
										 <?php
									             //Last week sql queries
									          
									            
                                                   
                                                      $sql_prepare = 'SELECT taggers.tagr_id, sum(tasks.attributes),sum(tasks.total_tags),sum(tasks.a_w_m),sum(tasks.t_w_m),sum(tasks.usd), taggers.username 
                                                                       FROM taggers INNER JOIN tasks ON taggers.tagr_id=tasks.tagr_id WHERE tasks.c_date BETWEEN "'.$dFrom.'" AND "'.$dTo.'" AND tasks.tagr_id=? AND taggers.tagr_type=1 AND taggers.status=1';  //join table tagger and task for getting tags do$dFrom_currentne by each taggger so far
										 ...
										 <?php
                            	                ...
                            	                $stmts = $con->prepare('SELECT task_id FROM tasks WHERE c_date BETWEEN "'.$dFrom.'" AND "'.$dTo.'" AND tagr_id=?'); //counting the number of task done by each tagger so far
